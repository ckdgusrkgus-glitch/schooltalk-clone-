...
<form action="student_process.php" method="post">
    ...
    <label>자녀 이름</label>
    <input type="text" name="name" placeholder="자녀 이름" required>
...
<h2 style="margin-top:40px;">자녀 이름</h2>
<table>
    <tr><th>이름</th><th>학년/반</th><th>처리</th></tr>
    <?php foreach ($students as $s): ?>
        <tr>
            <td><?= htmlspecialchars($s['name']) ?></td>
            <td><?= htmlspecialchars($s['class_name']) ?></td>
            <td>
                <form action="student_process.php" method="post" style="display:inline" onsubmit="return confirm('삭제하시겠습니까?')">
